...
{ 
    ...
    {
        int j;
        int part = reverse_byte_orderi(info->g_torrentmeta->info_hash[i]);
    ...
    strcpy(cur, "&port=");
    cur += strlen("&port=");
    cur += sprintf(cur, "%d", info->g_peer_port);

    // ip
    /* 
       strcpy(cur, "&ip=");
       cur += strlen("&ip=");
       strcpy(cur, info->g_my_ip);
       cur += strlen(info->g_my_ip);
       */
    // uploaded
    strcpy(cur, "&uploaded=");
    cur += strlen("&uploaded=");
    cur += sprintf(cur, "%d", info->g_uploaded);

    // downloaded
    strcpy(cur,"&downloaded=");
    cur += strlen("&downloaded=");
    cur += sprintf(cur, "%d", info->g_downloaded);
